...
const SearchProduct: React.FC<SearchProductProps> = ({ item }) => {
  ...
  const myLoader = ({ src }) => {
    return `http://207.244.250.143/day2day/public/assets/img/products/thumb/${item?.product_thumbnail}`;
  };
  return (
    <Link
    ...
    >
      <div className="relative flex w-24 h-24 rounded-md overflow-hidden bg-gray-200 flex-shrink-0 cursor-pointer me-4">
        <Image
          src={
            `http://207.244.250.143/day2day/public/assets/img/products/thumb/${item?.product_thumbnail}` ??
