//@ts-nocheck

import usePrice from "@framework/product/use-price";
import { useCart } from "@contexts/cart/cart.context";
import { CheckoutItem } from "@components/checkout/checkout-card-item";
import { CheckoutCardFooterItem } from "./checkout-card-footer-item";
import { useTranslation } from "next-i18next";
import { useContext } from "react";
import { colorsContext } from "@contexts/colors.context";

const CheckoutCard: React.FC = () => {
  const { items, total, isEmpty } = useCart();
  const { price: subtotal } = usePrice({
    amount: total,
    currencyCode: "USD",
  });
  const { theme } = useContext(colorsContext);
  const { t } = useTranslation("common");
  const checkoutFooter = [
    {
      id: 1,
      product_name: t("text-sub-total"),
      price: subtotal,
    },
    {
      id: 2,
      product_name: t("text-shipping"),
      price: t("text-free"),
    },
    {
      id: 3,
      product_name: t("text-total"),
      price: subtotal,
    },
  ];
  return (
    <div
      style={{
        color: theme.textColor,
      }}
      className="pt-12 md:pt-0 2xl:ps-4"
    >
      <h2 className="text-lg md:text-xl xl:text-2xl font-bold mb-6 xl:mb-8">
        {t("text-your-order")}
      </h2>
      <div
        style={{
          backgroundColor: theme.backgroundColorThird,
          color: theme.textColor,
        }}
        className="flex p-4 rounded-md mt-6 md:mt-7 xl:mt-9  text-sm font-semibold "
      >
        <span>{t("text-product")}</span>
        <span className="ms-auto flex-shrink-0">{t("text-sub-total")}</span>
      </div>
      {!isEmpty ? (
        items.map((item) => <CheckoutItem item={item} key={item.id} />)
      ) : (
        <p className="text-red-500 lg:px-3 py-4">{t("text-empty-cart")}</p>
      )}
      {checkoutFooter.map((item: any) => (
        <CheckoutCardFooterItem item={item} key={item.id} />
      ))}
    </div>
  );
};

export default CheckoutCard;
