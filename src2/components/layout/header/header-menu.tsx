...
const HeaderMenu: React.FC<MenuProps> = ({ className }) => {
  ...
  React.useEffect(() => {
    http.get(API_ENDPOINTS.MENU).then((response) => {
      setData(response.data.parentnav);
      //   console.log(response.data.parentnav, 'headerDATA');
    });
  }, []);
  const [subnewdata, subsetData] = React.useState([]);
  React.useEffect(() => {
    http.get(API_ENDPOINTS.SUBMENU).then((response) => {
      subsetData(response.data.subnav);
        console.log(response.data.subnav, 'subnav');
    });
  }, []);
  return (
